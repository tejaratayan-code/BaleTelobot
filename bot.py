...
async def callback_handler(client, callback_query):
    ...
    if data == "connect":
        ...
        # Set user state to wait for token
        # (We will handle it in message handler)
...
async def github_token_handler(client: Client, message: Message):
    ...
    if text.startswith("ghp_"):
        ...
        if test.status_code == 200:
            ...
            # Create GitelUpload repo if not exists
        ...
        # Normal message - ignore or handle as before
        pass
...
print("✅ ربات کامل و نهایی (با اتصال گیت‌هاب شخصی) در حال اجرا...")
